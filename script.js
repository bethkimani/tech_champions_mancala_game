/*document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateBoard() {
      for (let i = 0; i < 6; i++) {
          updatePocket('p1-pocket' + (i + 1), pits[i]); // Player 1's pockets
          updatePocket('p2-pocket' + (6 - i), pits[i + 6]); // Player 2's pockets
      }
  ...
  // Update a specific pocket with balls
  function updatePocket(pocketId, stoneCount) {
      const pocket = document.getElementById(pocketId);
      pocket.innerHTML = ''; // Clear previous content
      for (let i = 0; i < stoneCount; i++) {
          const ball = document.createElement('div');
          ball.classList.add('ball'); // Add class for styling
